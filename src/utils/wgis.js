...
var WGIS = (function () {
  ...
  // Check if a Leaflet layer is exportable
  ...
  function hasExportableChildren(group) {
    var found = false;
    if (group.eachLayer) {
      group.eachLayer(function (child) {
        if (isExportable(child)) { found = true; }
      });
    }
    return found;
  }

  self.hasExportableChildren = hasExportableChildren;
  ...
  // Parse a coordinate string directly into a 2D array
  ...
  self.parseCoordString = parseCoordString;
