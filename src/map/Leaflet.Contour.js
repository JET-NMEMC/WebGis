...
(function () {
  ...
  var convexHull = WGIS.convexHull;
  ...
  L.ContourLayer = L.Layer.extend({
    options: {
      ...
      opacity: 0.8,
      resolution: 500,
      maxCanvas: 2000,        // max canvas dimension (px) before DPI scaling
      showSites: false,       // show site markers (CircleMarker)
      pixelMode: false,
    ...
    _renderSites: function () {
      if (!this._map || !this.options.showSites) { return; }
      ...
      for (var i = 0; i < this._data.length; i++) {
        ...
        if (d.lng != null && d.lat != null) {
          ...
          marker.bindPopup((d.site || ('Site ' + i)) + ': ' + (d[valueField] !== undefined ? d[valueField] : 'N/A'));
          marker.addTo(sites);
        }
      }

      if (this._sitesLayer) {
        this._map.removeLayer(this._sitesLayer);
      }
      this._sitesLayer = sites;
      this._sitesLayer.addTo(this._map);
    },
    ...
    render: function () {
      ...
      // Reuse off-screen canvas, compute size dynamically
      var dpr = window.devicePixelRatio || 1;
      var canvasDim = Math.min(
        Math.max(grid.n, grid.m, 200) * 2,
        opts.maxCanvas || 2000
      );
      canvasDim = Math.round(canvasDim * dpr);
      if (!this._canvas) {
    ...
    onAdd: function (map) {
      ...
      if (this._sitesLayer) { this._sitesLayer.addTo(map); }
    },

    onRemove: function (map) {
      if (this._overlay) { map.removeLayer(this._overlay); }
      if (this._sitesLayer) { map.removeLayer(this._sitesLayer); }
      this._map = null;
    },
